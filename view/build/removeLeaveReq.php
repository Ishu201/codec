<script type="text/javascript">
const classList2 = document.querySelectorAll(".sweet-message");
  for (let i = 0; i < classList2.length; i++) {
    classList2[i].addEventListener("click", function() {
      const swalWithBootstrapButtons2 = Swal.mixin({
        customClass: {
          confirmButton: 'btn btn-success swal',
          cancelButton: 'btn btn-danger swal'
        },
        buttonsStyling: false
      })

      swalWithBootstrapButtons2.fire({
        title: 'Approve Leave',
        text: 'Are you Sure..?',
        icon: 'question',
        showCancelButton: false,
        confirmButtonText: 'Approve',
        cancelButtonText: 'Cancel',
        reverseButtons: true
      }).then((result) => {
        if (result.isConfirmed) {
          var leaveID = $(this).attr('data-id');
          var stat = $(this).attr('data-st');
          $.ajax({
            type: "GET", //we are using GET method to get data from server side
            url: '/codec/view/operation/processreq.php', // get the route value
            data: {leave_ID:leaveID,stat:stat}, //set data
            }).done(function (data) {
            console.log(data);

          swalWithBootstrapButtons2.fire(
            'Approved',
            'Leave Request is Processed',
            'success'
          ).then((result) => {
            location.href = 'leaveReq.php';
          });

          });

        } else if (result.dismiss === Swal.DismissReason.cancel) {
          swalWithBootstrapButtons2.fire(
            'Cancelled',
            'Record is Safe :)',
            'error'
          ).then((result)=>{
            location.href = 'leaveReq.php';
          });
        }

      });
    });
  };


  const classList1 = document.querySelectorAll(".sweet-message2");
    for (let i = 0; i < classList1.length; i++) {
      classList1[i].addEventListener("click", function() {
        const swalWithBootstrapButtons = Swal.mixin({
          customClass: {
            confirmButton: 'btn btn-danger swal',
            cancelButton: 'btn btn-danger swal'
          },
          buttonsStyling: false
        })

        swalWithBootstrapButtons.fire({
          title: "Reject Leave Request",
          text: 'Are you sure?',
          icon: 'question',
          showCancelButton: false,
          confirmButtonText: 'Reject',
          cancelButtonText: 'Cancel',
          reverseButtons: true
        }).then((result) => {
          if (result.isConfirmed) {
            var leaveID = $(this).attr('data-id');
            var stat = $(this).attr('data-st');
            $.ajax({
              type: "GET", //we are using GET method to get data from server side
              url: '/codec/view/operation/processreq.php', // get the route value
              data: {leave_ID:leaveID,stat:stat}, //set data
              }).done(function (data) {
              console.log(data);

            swalWithBootstrapButtons.fire(
              'Rejected',
              'Leave is Not Granted',
              'success'
            ).then((result) => {
              location.href = 'leaveReq.php';
            });

            });
          } else if (
            /* Read more about handling dismissals below */
            result.dismiss === Swal.DismissReason.cancel
          ) {
            swalWithBootstrapButtons.fire(
              'Cancelled',
              'Record is Safe :)',
              'error'
            )
          }
        });
      });
    };

</script>
