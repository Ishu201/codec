        ...
        <div class="main">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-8 p-r-0 title-margin-right">
                        <div class="page-header">
                            <div class="page-title"  id="taskTable">
                                <h1>Welcome <span>to Account Dashboard</span></h1>
                            </div>
                        </div>
                    </div>
                    <!-- /# column -->
                ...
                <section id="main-content">
                  ...
                  <?php
                    $sql_emp = "SELECT * FROM employee_loans WHERE status='pending'";
                  ...
                  <div class="row">
                          ...
                          <?php
                            ...
                            $sql_totEmp = "SELECT * FROM employee WHERE empJobStatus='Working'";
                            $result_totEmp = $con->query($sql_totEmp);
                            $tot_emp = 0;
                            while ($row_totEmp = $result_totEmp->fetch_array()) {
                              $get_sal_emp = $row_totEmp['empID'];
                              $sql_totEmp1 = "SELECT * FROM salary_master WHERE empID='$get_sal_emp' AND month='$month'";
                              $result_totEmp1 = $con->query($sql_totEmp1);
                              $count_totEmp1 = $result_totEmp1->num_rows;
                                if ($count_totEmp1 <= 0) {
                                  $tot_emp = $tot_emp + 1;
                                }
                            }
        									...
        									<div class="col-sm-3" style="padding:0px;">
        										<div class="card my">
        											<div class="card-body">
          												<div class="row">
          													<div class="col mt-0">
          														<h5 class="card-title">Salary Payment</h5>
          													</div>
          												...
          												<h2 class="mt-1 mb-3"><?php echo $tot_emp; ?></h2>
                                  <span class="text-muted">Remaining Payroll Generation</span>
        											</div>
        										</div>
                          </div>
                          ...
                          <?php
                          $date = date('Y');
                            $sql_expenses = "SELECT SUM(amount) FROM expenses WHERE paidDate LIKE '$date%'";
                            $result_expenses = $con->query($sql_expenses);
                            $row_expenses = $result_expenses->fetch_array();
                            $count_expenses = $row_expenses[0];
                            if ($count_expenses <= 0) {
                              $count_expenses = 0;
                            }
                          ?>
                          ...
                          <div class="col-sm-3" style="padding:0px;">
                                ...
                                <h4 class="mt-1 mb-3">Rs.<?php echo number_format($count_expenses,2); ?></h4>
                                <div class="mb-0" style="padding-top:8px;">
        													<span class="text-muted">Expenses in 2022</span>
        												</div>
        											</div>
        										</div>
        									</div>
                  ...
                  $sal = [];
                    ...
                    for ($i=1; $i < 13; $i++) {
                      ...
                      $expo_amt = 0;
                        $sql_expo = "SELECT SUM(totalPrice) FROM item_master WHERE purchaseDate LIKE '$month%'";
                          ...
                          <script type="text/javascript">
                            ...
                            var colorChangeValue = 50; //set this to whatever is the deciding color change value
                              ...
                              for (var i = 0; i < dataset.data.length; i++) {
                                if (dataset.data[i] < 100000) {
                                  dataset.backgroundColor[i] = chartColors.color1;
                                }
                                else if ((dataset.data[i] > 100000) && (dataset.data[i] <= 200000)){
                                  dataset.backgroundColor[i] = chartColors.color2;
                                }
                                else if ((dataset.data[i] > 200000) && (dataset.data[i] <= 500000)){
